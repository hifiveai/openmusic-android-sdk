...
import com.hfopenmusic.sdk.player.HifivePlayerView;
import com.hfopenmusic.sdk.ui.player.LifeFragmentManager;
...
public class HFLivePlayer {
    ...
    public HFLivePlayer showPlayer(FragmentActivity activity, int marginTop, int marginBottom) {
        ...
        synchronized (this) {
            ...
            //将播放器view添加为观察者
            HifiveMusicManage.getInstance().addObserver(mPlayerView);
    ...
    //移除播放器view,清除缓存数据
    public void destory() {
        HifiveMusicManage.getInstance().clearData();
        removePlayer();
    }
    //移除播放器view
